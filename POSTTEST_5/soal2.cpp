...
int findMinValue(Node* root) {
    ...
    // 1. Jika tree kosong, kembalikan -1 sesuai spesifikasi
    if (root == nullptr) {
        return -1;
    }

    // 2. Telusuri terus ke kiri sampai tidak ada anak kiri lagi
    Node* current = root;
    while (current->left != nullptr) {
        current = current->left;
    }

    // 3. current sekarang menunjuk ke node paling kiri → nilai terkecil
    return current->data;
    // -----------------------------
}
